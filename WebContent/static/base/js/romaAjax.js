...
function romaFieldChangedNoFocus(elementName){
	lastFocusName=null;
	romaAddChange(elementName);
}
...
function romaEventBg(elementName, eventName, params){
	var parName="(PojoEvent)_"+elementName+"_"+eventName;
	var request =romaBuildAjaxRequestData();
	if(params == undefined){
		request[parName]="event";// :P :P :P
	...
	romaSendAjaxRequestData(request, true, false);
}
...
function romaOnSuccess(data, textStatus) {
	...
	if(romaRespStatus=="stop"){
		...
		window.location.href = window.location.href.replace("/app", "/dynamic/common/logout.jsp");		// reload page
...
function updateAndSendLogin(userInputField, userPassField, loginAction) {
	romaFieldChangedNoFocus(userInputField);
	romaFieldChangedNoFocus(userPassField);
	romaAction(loginAction);
}
