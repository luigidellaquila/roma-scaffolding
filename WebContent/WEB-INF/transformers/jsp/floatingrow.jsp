...
%>
<div id="<%=JaniculumWrapper.id(component, null)%>" class="<%=JaniculumWrapper.cssClass(component, "row", null)%>" style="<%=JaniculumWrapper.inlineStyle(component, null)%>">
  <%
  int col = 0;
  for(Object c:JaniculumWrapper.getChildren(component)){
    ...
    if(label != null) {
  	...
  	%><span id="<%=JaniculumWrapper.id(component, null)%>_<%=col%>" class="col_<%=col %> <%=JaniculumWrapper.cssClass(child, null, null)%>"><%
      JspTransformerHelper.delegate(child, null,pageContext.getOut()); %></span>
  <%
