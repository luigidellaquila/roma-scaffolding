...
<table id="<%=JaniculumWrapper.id(component, null)%>" class="<%=JaniculumWrapper.cssClass(component, "rowset", null)%>" style="<%=JaniculumWrapper.inlineStyle(component, null)%>">
    ...
    <tr>
    <%} %>
    <td id="<%=JaniculumWrapper.id(component, null)%>_<%=row%>_td" class="<%=JaniculumWrapper.cssClass(child, null, null)%>"><%JspTransformerHelper.delegate(child, null,pageContext.getOut()); %></td>
    <%if(row % cols == cols - 1){%>
    </tr>
    <%} %>
   <%
