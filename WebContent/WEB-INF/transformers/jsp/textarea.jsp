...
%>
<div class="<%=JaniculumWrapper.cssClass(component, "textarea", null)%>"
	style="<%=JaniculumWrapper.inlineStyle(component, null)%>"
	id="<%=JaniculumWrapper.id(component, null)%>">
	<textarea id="<%=JaniculumWrapper.id(component, "content")%>"
		...
		on
		<%=event%>="romaFieldChanged('<%=JaniculumWrapper.fieldName(component)%>'); romaEvent('<%=JaniculumWrapper.fieldName(component)%>', '<%=event%>')"
		...
		<%}%>><%=JaniculumWrapper.content(component, true) == null ? "" : JaniculumWrapper.content(component, true).toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")%></textarea>
	<%
		if (!JaniculumWrapper.isValid(component)) {
	%>
	<span
		class="<%=JaniculumWrapper.cssClass(component, "textarea", "validation_message")%>"><%=JaniculumWrapper.validationMessage(component) == null ? "Invalid" : JaniculumWrapper.validationMessage(component)%></span>
	<%
		}
	%>
...
<%
	SchemaField field = ((HtmlViewContentComponent) component).getSchemaField();
		if (field.isSettedFeature(ValidationFieldFeatures.MAX)) {
			...
			if (max != null && max > 0) {
				String js = "jQuery('#" + JaniculumWrapper.id(component, "content") + "').keyup(function(){" + "	var value = \"\"+jQuery('#"
						+ JaniculumWrapper.id(component, "content") + "').val();" + "	if(value.length>" + max + "){" + "   	jQuery('#" + JaniculumWrapper.id(component, "content")
						+ "').val(value.substring(0, " + max + "));" + "	}" + "	romaFieldChanged('${janiculum.fieldName()}');" + "});";
				JspTransformerHelper.addJs(JaniculumWrapper.id(component, TransformerConstants.PART_ALL), js);
			}
		}
%>
...
%><label id="<%=JaniculumWrapper.id(component, "label")%>"
	class="<%=JaniculumWrapper.cssClass(component, "textarea", "label")%>"
