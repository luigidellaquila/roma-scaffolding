	...
	<div class="<%=JaniculumWrapper.cssClass(component, "decimal", null)%>" style="<%=JaniculumWrapper.inlineStyle(component, null)%>" id="<%=JaniculumWrapper.id(component, null)%>">
		...
		for(String event:JaniculumWrapper.availableEvents(component)){
			if("change".equals(event)){
				...
				if((request.getSession().getAttribute(CustomApplicationConfiguration.IS_IE_SESSION_PARAMETER) != null && ((Boolean)request.getSession().getAttribute(CustomApplicationConfiguration.IS_IE_SESSION_PARAMETER)))) {
				%>onblur="romaFieldChangedNoFocus('<%=JaniculumWrapper.fieldName(component)%>'); romaEvent('<%=JaniculumWrapper.fieldName(component)%>', '<%=event%>')"<%	
				}
			}
		%>
		...
		<%
			...
			if(!existsChangeEvent || (request.getSession().getAttribute(CustomApplicationConfiguration.IS_IE_SESSION_PARAMETER) != null && ((Boolean)request.getSession().getAttribute(CustomApplicationConfiguration.IS_IE_SESSION_PARAMETER)))) {
			%> onblur="romaFieldChangedNoFocus('<%=JaniculumWrapper.fieldName(component)%>')"<%		
