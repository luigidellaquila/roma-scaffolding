...
<div title="<%=JaniculumWrapper.i18NHint(component)%>" class="<%=JaniculumWrapper.cssClass(component, "label", null)%>" style="<%=JaniculumWrapper.inlineStyle(component, null)%>" id="<%=JaniculumWrapper.id(component, null)%>" 
	...
	>
	 <span id="<%=JaniculumWrapper.id(component, "content")%>"  >
    	<%=JaniculumWrapper.formattedContent(component)==null ? "" : JaniculumWrapper.formattedContent(component).toString().replaceAll("<", "&lt;").replaceAll(">", "&gt;")%>
	 </span>
</div>
<% 
	}
